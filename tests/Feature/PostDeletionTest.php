<?php
// tests/Feature/PostDeletionTest.php

namespace Tests\Feature;

use App\Models\Post;
use App\Models\User;
use Illuminate\Foundation\Testing\RefreshDatabase;
use Illuminate\Foundation\Testing\WithFaker;
use Livewire\Livewire;
use Tests\TestCase;

class PostDeletionTest extends TestCase
{
    use RefreshDatabase;

    /**
     * Test that a user can delete their own post.
     */
    public function test_user_can_delete_their_own_post()
    {
        // Create a user and log them in
        $user = User::factory()->create();
        $this->actingAs($user);

        // Create a new post
        $post = Post::factory()->create(['user_id' => $user->id]);

        // Visit the post page
        $response = $this->get(route('posts.edit', $post->id));
        $response->assertStatus(200);

        // Delete the post using Livewire
        Livewire::test('add-post', ['post_id' => $post->id])
            ->call('delete')
            ->assertRedirect(route('dashboard'));

        // Assert that the post was deleted successfully
        $this->assertDatabaseMissing('posts', [
            'id' => $post->id,
        ]);
    }

    /**
     * Test that a user cannot delete another user's post.
     */
    public function test_user_cannot_delete_another_users_post()
    {
        // Create two users
        $user1 = User::factory()->create();
        $user2 = User::factory()->create();

        // Create a new post by user2
        $post = Post::factory()->create(['user_id' => $user2->id]);

        // Log in as user1
        $this->actingAs($user1);

        // Visit the post page
        $response = $this->get(route('posts.edit', $post->id));
        $response->assertStatus(403);

        // Assert that the post was not deleted
        $this->assertDatabaseHas('posts', [
            'id' => $post->id,
        ]);
    }

    /**
     * Test that a guest user cannot delete a post.
     */
    public function test_guest_user_cannot_edit_a_post()
    {
        // Create a new post
        $post = Post::factory()->create();

        // Visit the post page as a guest user
        $response = $this->get(route('posts.edit', $post->id));
        $response->assertStatus(302);
    }

    /**
     * Test that a user cannot delete a non-existent post.
     */
    public function test_user_cannot_access_a_non_existent_post()
    {
        // Create a user and log them in
        $user = User::factory()->create();
        $this->actingAs($user);

        // Attempt to delete a non-existent post using Livewire
        $response = $this->get(route('posts.edit', 999));
        $response->assertStatus(404);
    }
}
